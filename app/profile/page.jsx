  ...
  return (
    <div className="mt-8 grid gap-4">
      ...
      <div className="rounded-xl bg-amber-50 border border-amber-200 p-4 flex items-center gap-3">
        ...
        <span className="text-sm text-amber-700">You&apos;re viewing a demo profile. <Link href="/login" className="font-semibold underline">Sign in</Link> to manage your account.</span>
      </div>
        ...
        <div className="space-y-6">
          <div className="flex justify-center py-2">
            <div className="flex flex-col items-center gap-3">
              <div className="h-24 w-24 rounded-full bg-brand grid place-items-center text-white font-bold text-2xl">D</div>
              <div className="text-xs text-muted">Click to upload avatar</div>
            </div>
          </div>
          <div className="space-y-4">
            ...
            <label className="block">
              ...
              <input readOnly value="Demo User" className="input mt-1.5 opacity-60 cursor-not-allowed" />
            </label>
          </div>
          <div className="flex items-center gap-3 pt-2">
            <Link href="/login" className="btn-primary px-6">Sign in to edit</Link>
          </div>
        </div>
      </div>
      ...
      <div className="card p-6">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="h-10 w-10 rounded-xl bg-[#1DB954]/10 text-[#1DB954] grid place-items-center">
            ...
            <div>
              ...
              <div className="text-sm text-muted mt-0.5">Connected as <span className="text-text font-medium">Demo User</span> &middot; since 4/10/2026</div>
            </div>
          </div>
          <Link href="/api/spotify/connect" className="btn-secondary">Reconnect</Link>
        </div>
      </div>
...
export default async function ProfilePage() {
  ...
  try {
    ...
    if (authUser) {
      ...
      profile = p;
      const { data: s } = await supabase.from("spotify_accounts")
